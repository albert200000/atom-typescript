...
if (packageJson.configSchema) {
  ...
  // If the file is an ES module you can import it dynamically:
  // const mod = await import(filePath)  // only allowed inside an async function
  // config = mod.config
  // For compatibility with CommonJS-style modules on the filesystem use require:
  config = require(filePath).config
  outfile = readFileSync(filePath)
}
